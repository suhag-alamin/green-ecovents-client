...
import { IBooking, IEvent, IUser, IUserInfo } from "@/interfaces/global";
...
const BookingForm = ({ event, user, id }: IBookingFormProps) => {
  ...
  const handleBookEvent = async (data: any) => {
    if (user?.id && id) {
      setIsLoading(true);
      const res = await axiosInstance.post("bookings/create-payment-intents", {
  ...
  return (
    <div className="">
      <Row gutter={[20, 20]}>
        <Col xs={24} lg={isProceedToPayment ? 12 : 24}>
          <div
            style={{
